...
from dcgpen.training.data_loader import degradations
...
class GFPGAN_degradation(object):
    ...
    def degrade_process(self, img_gt):
        ...
        if np.random.uniform() < self.color_jitter_prob:
            jitter_val = np.random.uniform(-self.shift, self.shift, 3).astype(
                np.float32
            )
...
class FaceDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_gt = cv2.resize(
            img_gt, (self.resolution, self.resolution), interpolation=cv2.INTER_AREA
        )
