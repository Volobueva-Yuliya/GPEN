...
from dcgpen.face_inpainting import FaceInpainting
from dcgpen.segmentation2face import Segmentation2Face
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--tile_size", type=int, default=0, help="tile size for SR to avoid OOM"
    )
    parser.add_argument(
    ...
    if args.task == "FaceEnhancement":
        ...
        processer = FaceColorization(
            in_size=args.in_size,
            model=args.model,
            device="cuda" if args.use_cuda else "cpu",
        )
    elif args.task == "FaceInpainting":
        processer = FaceInpainting(
            in_size=args.in_size,
            model=args.model,
            device="cuda" if args.use_cuda else "cpu",
        )
    elif args.task == "Segmentation2Face":
        processer = Segmentation2Face(
            in_size=args.in_size,
            model=args.model,
            is_norm=False,
            device="cuda" if args.use_cuda else "cpu",
    ...
    for n, file in enumerate(files[:]):
        ...
        cv2.imwrite(
            os.path.join(
                args.outdir, ".".join(filename.split(".")[:-1]) + f"_COMP{args.ext}"
            ),
            np.hstack((img, img_out)),
        )
        cv2.imwrite(
            os.path.join(
                args.outdir, ".".join(filename.split(".")[:-1]) + f"_GPEN{args.ext}"
            ),
            img_out,
        )
        ...
        if args.save_face:
            for m, (ef, of) in enumerate(zip(enhanced_faces, orig_faces)):
                ...
                cv2.imwrite(
                    os.path.join(
                        args.outdir,
                        ".".join(filename.split(".")[:-1]) + "_face%02d" % m + args.ext,
                    ),
                    np.hstack((of, ef)),
                )
